...
Mule supports very many internet communications protocols out of the box, such as FTP and SMTP. Over time, the community of developers has solidified Mule's reputation as a connection-aware runtime. Mule applications support use and re-use of elements called *Anypoint Connectors* that facilitate hooking applications into external web services. See the whole connector offering here on *Anypoint Exchange*.
...
To elaborate on what design and development for business transformation means, consider the data you want to provide, govern, transform, and expose at various levels of your organization, both internally and customer-facing.
...
*API Designer*, accessible from the browser, or *APIkit*, available within Studio. We support APIs defined in any format, promoting API development segmented into three logical layers of your organization:
...
Developing an application network with the Anypoint Platform is no small task. In fact, it is a gradual process that involves many people and resources - but the process does not have to be painful. Many teams working in parallel to create an API architecture that can be updated as organizational demands change is a crucial undertaking for business survival, but before you begin, you have to know the context in which your data and applications exist - the details that define where your service model is today, and where it needs to go in order to meet the demands of your organization and customers now, and the group of users you want to connect and engage with tomorrow.
